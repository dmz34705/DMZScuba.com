(() => {
  ...
  function offeringEditor(item = {}) {
    ...
    const shortDescription = editor.querySelector("textarea[name=description]");
    shortDescription?.insertAdjacentHTML("afterend", `<label class="mgmt-booking-long-description"><span>Long-form event page</span><small>Paste formatted content here. Basic formatting is retained on the public Learn More page.</small><div class="mgmt-booking-rich-editor" contenteditable="true" data-long-description>${item.longDescriptionHtml || ""}</div></label>`);
  ...
  editor.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (event.target.matches("[data-admin-offering-form]")) {
        ...
        offering.longDescriptionHtml = event.target.querySelector("[data-long-description]")?.innerHTML || "";
