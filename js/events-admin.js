(() => {
  ...
  function fillForm(entry) {
    ...
    if (entry.kind === "event") {
      setFocus(`Editing ${item.date || "new date draft"}`);
      setModalNote(`Editing the one-time event for ${item.date || "this date"}.`);
      return;
    }

    setFocus(`Editing recurring template: ${item.title || item.id || "untitled"}`);
    setModalNote(`Editing the recurring template "${item.title || item.id || "untitled"}".`);
  }
  ...
  if (addTemplateBtn) {
    addTemplateBtn.addEventListener("click", () => createEntry("template"));
  }

  if (addEventBtn) {
    addEventBtn.addEventListener("click", () => createEntry("event"));
  }
