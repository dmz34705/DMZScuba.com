(function () {
  ...
  function fitCanvas() {
    ...
    const targetW = Math.min(920, maxW);

// Mobile = square, Desktop = cinematic
const isMobile = window.innerWidth <= 768;
const targetH = isMobile ? targetW : Math.round(targetW * 0.56);
  ...
  function drawMappedEarth(cx, cy, r) {
    ...
    const bufSize = Math.max(220, Math.min(520, Math.round(r * 2)));
...
const cx = w * 0.5;
const cy = window.innerWidth <= 768 ? h * 0.5 : h * 0.52;
...
function handlePinTapMobile(x, y) {
  ...
  // Copy destination name (if button exists)
  const copyBtn = document.getElementById("copyDest");
  if (copyBtn) {
    copyBtn.addEventListener("click", async () => {
      const name = (document.getElementById("destTitle")?.textContent || "").trim();
      try {
        await navigator.clipboard.writeText(name);
        copyBtn.textContent = "Copied!";
        setTimeout(() => (copyBtn.textContent = "Copy Destination Name"), 900);
      } catch {
        window.prompt("Copy this:", name);
      }
    });
  }
